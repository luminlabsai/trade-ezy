...
def get_or_create_sender_id(instagram_sender_id: int) -> str:
    """ Fetch or create a mapped UUID (sender_id) for an Instagram sender ID. """
    try:
        ...
        if result:
            ...
            # Create a new user entry and return the sender_id
            cur.execute(
                """
                INSERT INTO users (instagram_sender_id) 
                VALUES (%s) 
                RETURNING sender_id
                """,
                (instagram_sender_id,)
